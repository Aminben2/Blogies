...
import { updatePersonnelInfo } from "../../store/usersSlice";
import { useToast } from "@chakra-ui/react";

function UpdatePersonnalInfo({ setShow }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!response.ok) {
      ...
      dispatch(updatePersonnelInfo({ ...data, userId: user._id }));
      setShow();
      toast({
        title: `Personnal info are updated`,
        status: "success",
        isClosable: true,
      });
      setData({
